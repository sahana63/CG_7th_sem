...
// Clips all the edges of polygon taking one clip edge of clipping window
void clip(int poly_points[][2], int *poly_size, int x1, int y1, int x2, int y2)
{
	...
	for (int i = 0; i < *poly_size; i++)
	{
		...
		if (i_pos >= 0 && k_pos >= 0)
		{
			//Only second point is added (in, in)
			new_points[new_poly_size][0] = kx;
			new_points[new_poly_size][1] = ky;
			new_poly_size++;
		}

		// Case 2: When only second point is inside (out, in)
		...
		// Case 3: When only first point is inside (in, out) 
		else if (i_pos >= 0 && k_pos < 0)
		{
		...
		// Case 4: When both points are outside (out, out)
		else
		{
			//No points are added 
		}
...
// Implements Sutherland Hodgeman Polygon Clipping algorithm 
void display()
{
	init();
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Sutherman Hodgeman Polygon Clipping");
